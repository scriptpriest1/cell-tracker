<div id="action-modal" class="action-modal position-fixed top-0 start-0 end-0 bottom-0 w-100 h-100 m-0 p-0">
  <div class="modal-overlay w-100 h-100 m-0 p-0 position-relative">
    <div class="action-panel position-absolute">

      <header class="header d-flex align-items-center gap-2 justify-content-between px-4 py-3 position-absolute top-0 w-100">
        <div class="title-bar d-flex align-items-center gap-2 position-relative">
          <h5 class="title m-0 p-0">Title</h5>
          <div id="edit-title-container"></div>
        </div>
        <div class="action-bar">
          <button id="sidebar" class="p-0 m-0 bg-none load-action-modal-dyn-content" data-content-type="edit-cell-admin">Sidebar</button>
          <button type="button" class="close-btn p-0 m-0">
            <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="#000000">
              <path d="M480-424 284-228q-11 11-28 11t-28-11q-11-11-11-28t11-28l196-196-196-196q-11-11-11-28t11-28q11-11 28-11t28 11l196 196 196-196q11-11 28-11t28 11q11 11 11 28t-11 28L536-480l196 196q11 11 11 28t-11 28q-11 11-28 11t-28-11L480-424Z"/>
            </svg>
          </button>
        </div>
      </header>

      <div class="panel-body m-0 p-0 pt-1 w-100 h-100 position-relative">
        <div class="content-container mt-1 pt-5"></div>
        <div class="side-panel position-absolute w-100 pt-5"></div>
      </div>

      <footer class="footer d-none px-4 py-2 position-absolute bottom-0 w-100"></footer>
    </div>
  </div>
</div>